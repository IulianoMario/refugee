WITH
  ...
  from `data-warehouse-229515.wunderflats_core_refugee.listing_requests`
    ...
    pipedrive_bookings as (
        ...
        case when pd.email is not null and prd.email is not null then prd_number_of_tenants 
             when pd.email is null and prd.email is not null then prd_number_of_tenants
             when pd.email is not null and prd.email is null then pd_number_of_tenants 
