   WITH
    ...
    END
      ...
      0
    END
      AS ll_nights_per_rent_duration,
      CASE
    ...
    END
      ...
      CASE
        WHEN LOWER(te_pets_cleaned) LIKE '%no%' OR
    ...
    IF
      ...
      _te_needed_months_to_rent_,
      case when _te_needed_months_to_rent_ = '<1  month' then '<1 mo'
         when _te_needed_months_to_rent_ = '<1 Monat' then '<1 mo'
         when _te_needed_months_to_rent_ = '<1 month' then '<1 mo'
         when _te_needed_months_to_rent_ = '<1 months' then '<1 mo'
         when _te_needed_months_to_rent_ = '<1 мonth' then '<1 mo'
         when _te_needed_months_to_rent_ = '<1 месяц' then '<1 mo'
         when _te_needed_months_to_rent_ = '<1 місяць' then '<1 mo'
         when _te_needed_months_to_rent_ = '<1month' then '<1 mo'
         when _te_needed_months_to_rent_ = '1' then '1 mo'
         when _te_needed_months_to_rent_ = '1 month' then '1 mo'
         when _te_needed_months_to_rent_ = '1 month,' then '1 mo'
         when _te_needed_months_to_rent_ = '1 to 3 months' then '1-3 mo'
        when _te_needed_months_to_rent_ = '1-2 months' then '1-3 mo'
        when _te_needed_months_to_rent_ = '1-3' then '1-3 mo'
        when _te_needed_months_to_rent_ = '1-3 months' then '1-3 mo'
        when _te_needed_months_to_rent_ = '1–3 Monat' then '1-3 mo'
        when _te_needed_months_to_rent_ = '1–3 Monate' then '1-3 mo'
        when _te_needed_months_to_rent_ = '1–3 month' then '1-3 mo'
        when _te_needed_months_to_rent_ = '1–3 monthes' then '1-3 mo'
        when _te_needed_months_to_rent_ = '1–3 months' then '1-3 mo'
        when _te_needed_months_to_rent_ = '1–3 мonate' then '1-3 mo'
        when _te_needed_months_to_rent_ = '1–3 мonths' then '1-3 mo'
        when _te_needed_months_to_rent_ = '1–3 місяці' then '1-3 mo'
        when _te_needed_months_to_rent_ = '1–3 місяці / 1-3 months' then '1-3 mo'
        when _te_needed_months_to_rent_ = '1–3months' then '1-3 mo'
        when _te_needed_months_to_rent_ = '2' then '1-3 mo'
        when _te_needed_months_to_rent_ = '3 - 6 months' then '3-6 mo'
        when _te_needed_months_to_rent_ = '3 to 6 months' then '3-6 mo'
        when _te_needed_months_to_rent_ = '3-6 Monate' then '3-6 mo'
        when _te_needed_months_to_rent_ = '3-6 month' then '3-6 mo'
        when _te_needed_months_to_rent_ = '3-6 months' then '3-6 mo'
        when _te_needed_months_to_rent_ = '3-6 months ' then '3-6 mo'
        when _te_needed_months_to_rent_ = '3–6 Monate' then '3-6 mo'
        when _te_needed_months_to_rent_ = '3–6 month' then '3-6 mo'
        when _te_needed_months_to_rent_ = '3–6 months' then '3-6 mo'
        when _te_needed_months_to_rent_ = '3–6 мonths' then '3-6 mo'
        when _te_needed_months_to_rent_ = '3–6 місяців' then '3-6 mo'
        when _te_needed_months_to_rent_ = "As long as it's not in" then 'Unknown'
        when _te_needed_months_to_rent_ = "As long as it's not in dotted" then 'Unknown'
        when _te_needed_months_to_rent_ = "As long as it's not in text" then 'Unknown'
        when _te_needed_months_to_rent_ = "as long as possible" then 'Unknown'
        when _te_needed_months_to_rent_ = "As long as required" then 'Unknown'
        when _te_needed_months_to_rent_ = "Between 1 and 3 Months" then '1-3 mo'
        when _te_needed_months_to_rent_ like "%know%" then 'Unknown'
        when _te_needed_months_to_rent_ like "%indef%" then '>6 mo'
        when _te_needed_months_to_rent_ like "%indef%" then '>6 mo'
        when _te_needed_months_to_rent_ = "From 1 - 3 months" then '1-3 mo'
        when _te_needed_months_to_rent_ = "from 1 to 3 months" then '1-3 mo'
        when _te_needed_months_to_rent_ = "from 1 tо 3 months" then '1-3 mo'
        when _te_needed_months_to_rent_ = "from 3 to 6 month" then '3-6 mo'
        when _te_needed_months_to_rent_ = "from 3 to 6 months" then '3-6 mo'
        when _te_needed_months_to_rent_ = "I am not sure" then 'Unknown'
        when _te_needed_months_to_rent_ = "I can't pay" then 'Unknown'
        when _te_needed_months_to_rent_ = "I can’t pay rent" then 'Unknown'
        when _te_needed_months_to_rent_ = "less a month" then '1 mo'
        when _te_needed_months_to_rent_ = "less than 1 month" then '1 mo'
        when _te_needed_months_to_rent_ = "Less than 1 month - <1 місяць" then '1 mo'
        when _te_needed_months_to_rent_ = "less than a month" then '1 mo'
        when _te_needed_months_to_rent_ = "Less than a month - <1 місяць" then '1 mo'
        when _te_needed_months_to_rent_ = "less than a month," then '1 mo'
        when _te_needed_months_to_rent_ = "less than one month" then '1 mo'
        when _te_needed_months_to_rent_ = "Not defined yet" then 'Unknown'
        when _te_needed_months_to_rent_ = "Not sure" then 'Unknown'
        when _te_needed_months_to_rent_ = "Not sure so far, я ще не знаю" then 'Unknown'
        when _te_needed_months_to_rent_ = "Not sure yet." then 'Unknown'
        when _te_needed_months_to_rent_ = "open." then 'Unknown'       
        when _te_needed_months_to_rent_ = "s" then 'Unknown'  
        when _te_needed_months_to_rent_ = "So far, no strings attached" then '>6 mo'  
        when _te_needed_months_to_rent_ = "So far, no strings attached." then '>6 mo' 
        when _te_needed_months_to_rent_ = "So far, no strings attached." then '>6 mo' 
        when _te_needed_months_to_rent_ = "So far, no strings attached.ž" then '>6 mo' 
        when _te_needed_months_to_rent_ = "So long as it's in no particular order" then '>6 mo' 
        when _te_needed_months_to_rent_ = "Uncertain" then '>6 mo' 
        when _te_needed_months_to_rent_ = "Undetermined" then '>6 mo' 
        when _te_needed_months_to_rent_ = "Unlimited term" then '>6 mo' 
        when _te_needed_months_to_rent_ = "от 1 3 months" then '1-3 mo' 
        when _te_needed_months_to_rent_ = "от 1 до 3 месяцев" then '1-3 mo' 
        when _te_needed_months_to_rent_ = "от 3 до 6 месяцев" then '3-6 mo' 
        when _te_needed_months_to_rent_ = "Пока на неопределенный срок" then '>6 mo' 
        when _te_needed_months_to_rent_ = "Поки що безстроково" then '>6 mo' 
        when _te_needed_months_to_rent_ = "Я еще не знаю" then 'Unknown' 
        when _te_needed_months_to_rent_ = "я ще не знаю" then 'Unknown' 
        else "Unknown" end as rent_duration_categorical,
    ...
    SELECT
      *, 
      case when rent_duration_categorical = '<1 mo' then  15
         when rent_duration_categorical = '1 mo' then 30
         when rent_duration_categorical = '1-3 mo' then  60
         when rent_duration_categorical = '3-6 mo' then  120
         when rent_duration_categorical = '>6 mo' then  360
  ...
  from `data-warehouse-229515.wunderflats_core_refugee.bookings` rfg_bk
  left join `data-warehouse-229515.wunderflats_core.listings`   core_ls
    ...
    SELECT
      DISTINCT listing_id,
      pets_allowed
    from `data-warehouse-229515.wunderflats_core_refugee.listing_requests`
  ),
      bookings_refugee AS (
      SELECT
          rfg_bk.booking_id,
          ...
          rfg_bk.number_of_tenants as prd_number_of_tenants,
      ...
      LEFT JOIN `data-warehouse-229515.wunderflats_core_refugee.listings`  rfg_lis
      ...
      on users.user_id = rfg_bk.tenant_id
  ),
      pipedrive_bookings as (
          ...
          te_number_of_people_in_the_group_keep_updated as pd_number_of_tenants,
          city,
          ...
          case when pd.email is not null and prd.te_email  is not null then prd.te_email
              when pd.email is null and prd.te_email  is not null then prd.te_email
              when pd.email is not null and prd.te_email  is null then pd.email
              end as te_email,
          ...
          case when pd.rent_duration_categorical is null and prd.nights_per_rent_duration < 30 then "<1 mo"
              when pd.rent_duration_categorical is null and prd.nights_per_rent_duration = 30 then "1 mo"
              when pd.rent_duration_categorical is null and (prd.nights_per_rent_duration >30 and prd.nights_per_rent_duration < 90 ) then "1-3 mo"
              when pd.rent_duration_categorical is null and (prd.nights_per_rent_duration > 90 and prd.nights_per_rent_duration < 180) then "3-6 mo"
              when pd.rent_duration_categorical is null and prd.nights_per_rent_duration >= 180 then ">6 mo"
              when pd.rent_duration_categorical is not null then ll_max_rent_duration else 'Unknown' end as max_rent_duration_categorical,
           _te_needed_months_to_rent_ as pd_te_rent_duration_raw,
          ...
          case when pd.email is not null and prd.te_email is not null then prd_number_of_tenants
              when pd.email is null and prd.te_email is not null then prd_number_of_tenants
              when pd.email is not null and prd.te_email is null then pd_number_of_tenants
          ...
          case when pd.id is not null or pd.id is null then prd.is_published end as is_published
          from pipedrive_bookings pd
          full join bookings_refugee prd
          on pd.email = prd.te_email
      ),
          final as (
              Select
              source_flag,
