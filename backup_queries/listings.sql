...
Select
    ...
    city as city,
    --(Select city from unnest (a.ll_cities_cleaned) city limit 1) as city,
    deleted,
    owner_name,
    cc_email,
    ...
    te_priority,
       ...
       case when _ll_maximum_rent_duration_possible_ in ('Less than one month','Unter einem Monat') then 29
        ...
        CASE WHEN LOWER(te_pets_cleaned) LIKE '%no %' OR LOWER(te_pets_cleaned) LIKE '%without' OR LOWER(te_pets_cleaned) LIKE 'none%' THEN FALSE 
...
and email.primary = true
    ...
    ls.is_published,
    ls.is_deleted
    from `data-warehouse-229515.wunderflats_core_refugee.landlords` ll
    right join `data-warehouse-229515.wunderflats_core_refugee.listings` ls
    ...
    product_data_final as (
       ...
       prd.created_at as prd_created_at,
        ...
        open_deal as pd_open_deal, 
        lost_deal as pd_lost_deal,
        won_deal as pd_won_deal
