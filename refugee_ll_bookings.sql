WITH
  ...
  FROM
    `data-warehouse-229515.staging.pipedrive_refugee_deals` a
  LEFT JOIN
  ...
  SELECT
    ...
    core_ls.is_deleted,
    core_ls.created_at,
    --date_diff(current_date(), date(core_ls.created_at), day) as prd_nights_from_won_date, -- why do we need this one?
  ...
  FROM `data-warehouse-229515.wunderflats_core_refugee.listing_requests`
    ...
    SELECT
        ...
        rfg_bk.is_canceled,
        rfg_bk.listing_id,
        -- Does it matter to have a listing created_at column ? Shall we not have the booking created at date? 
        --core_bk.created_at as listing_created_at,
    ...
    left join `data-warehouse-229515.wunderflats_core_refugee.landlords` rfg_ll
    ...
    pipedrive_bookings as (
        ...
        --booked_from not available, 
        --booked_to not available, 
        --is_cancelled not available, 
        --listing_id not available,
        --is_online not available, 
        --is_bookable not available, 
        --is_published not available,   
        deleted as is_deleted,
         --listing_available_from not available,
        --listing_available_to not available, 
        ...
        city,
        --area not available, 
        --next_available_date not available, 
        ...
        case when ll_max_rent_duration is null and prd.nights_per_rent_duration < 30 then "<1 mo"
             ...
             when ll_max_rent_duration is not null then ll_max_rent_duration end as max_rent_duration_cat,
        ...
        case when prd.price = 0 then 'for_free'
             ...
             else null end as prd_rent_per_person_cat,
